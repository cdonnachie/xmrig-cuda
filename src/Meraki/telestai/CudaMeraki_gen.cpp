...
static std::mutex Meraki_cache_mutex;
static std::mutex Meraki_build_mutex;
static std::vector<CacheEntry> Meraki_cache;
...
static void Meraki_build_program(
    std::vector<char>& ptx,
    std::string& lowered_name,
    uint64_t period,
    int arch_major,
...
{
    {
        std::lock_guard<std::mutex> g(Meraki_cache_mutex);

        // Remove old programs from cache
        for (size_t i = 0; i < Meraki_cache.size();) {
            const CacheEntry& entry = Meraki_cache[i];
            if (entry.period + 2 < period) {
                Meraki_cache[i] = std::move(Meraki_cache.back());
                Meraki_cache.pop_back();
            }
            else {
                ++i;
            }
    ...
    nvrtcProgram prog;
    nvrtcResult result = nvrtcCreateProgram(&prog, source.c_str(), "Meraki.cu", 0, nullptr, nullptr);
    ...
    {
        std::lock_guard<std::mutex> g(Meraki_cache_mutex);
        Meraki_cache.emplace_back(period, arch_major, arch_minor, ptx, lowered_name);
    }
}
...
void Meraki_get_program(std::vector<char>& ptx, std::string& lowered_name, uint64_t period, uint32_t threads, int arch_major, int arch_minor, const uint64_t* dag_sizes, bool background)
{
    if (background) {
        background_exec([=]() { std::vector<char> tmp; std::string s; Meraki_get_program(tmp, s, period, threads, arch_major, arch_minor, dag_sizes, false); });
        return;
    }
    ...
    Meraki_build_program(ptx, lowered_name, period, arch_major, arch_minor, source_code);
}
