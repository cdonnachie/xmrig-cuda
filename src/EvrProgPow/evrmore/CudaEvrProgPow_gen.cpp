...
static std::mutex EvrProgPow_cache_mutex;
static std::mutex EvrProgPow_build_mutex;
static std::vector<CacheEntry> EvrProgPow_cache;
...
void EvrProgPow_calculate_fast_mod_data(uint32_t divisor, uint32_t& reciprocal, uint32_t& increment, uint32_t& shift)
{
...
static void EvrProgPow_build_program(
    std::vector<char>& ptx,
    std::string& lowered_name,
    uint64_t period,
    int arch_major,
...
{
    {
        std::lock_guard<std::mutex> g(EvrProgPow_cache_mutex);

        // Remove old programs from cache
        for (size_t i = 0; i < EvrProgPow_cache.size();) {
            const CacheEntry& entry = EvrProgPow_cache[i];
            if (entry.period + 2 < period) {
                EvrProgPow_cache[i] = std::move(EvrProgPow_cache.back());
                EvrProgPow_cache.pop_back();
            }
            else {
                ++i;
            }
    ...
    std::lock_guard<std::mutex> g1(EvrProgPow_build_mutex);
    {
        std::lock_guard<std::mutex> g(EvrProgPow_cache_mutex);

        // Check if the cache already has this program (some other thread might have added it first)
        for (const CacheEntry& entry : EvrProgPow_cache)
        {
    ...
    nvrtcProgram prog;
    nvrtcResult result = nvrtcCreateProgram(&prog, source.c_str(), "EvrProgPow.cu", 0, nullptr, nullptr);
    ...
    {
        std::lock_guard<std::mutex> g(EvrProgPow_cache_mutex);
        EvrProgPow_cache.emplace_back(period, arch_major, arch_minor, ptx, lowered_name);
    }
}
...
void EvrProgPow_get_program(std::vector<char>& ptx, std::string& lowered_name, uint64_t period, uint32_t threads, int arch_major, int arch_minor, const uint64_t* dag_sizes, bool background)
{
    if (background) {
        background_exec([=]() { std::vector<char> tmp; std::string s; EvrProgPow_get_program(tmp, s, period, threads, arch_major, arch_minor, dag_sizes, false); });
        return;
    }
    ...
    EvrProgPow_build_program(ptx, lowered_name, period, arch_major, arch_minor, source_code);
}
