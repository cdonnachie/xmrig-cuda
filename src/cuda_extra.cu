...
#   endif
    ...
    if (algorithm.family() != Algorithm::KAWPOW && algorithm.family() != Algorithm::MEOWPOW && algorithm.family() != Algorithm::EVRPROGPOW) {
...
int cuda_get_deviceinfo(nvid_ctx *ctx)
{
    ...
    if ((ctx->algorithm.family() == Algorithm::KAWPOW || ctx->algorithm.family() == Algorithm::MEOWPOW || ctx->algorithm.family() == Algorithm::EVRPROGPOW) && ((ctx->device_blocks < 0) || (ctx->device_threads < 0))) {
