...
#   endif
    ...
    if (algorithm.family() != Algorithm::KAWPOW && algorithm.family() != Algorithm::MEOWPOW && algorithm.family() != Algorithm::EVRPROGPOW && algorithm.family() != Algorithm::MERAKI) {
...
int cuda_get_deviceinfo(nvid_ctx *ctx)
{
    ...
    if ((ctx->algorithm.family() == Algorithm::KAWPOW || ctx->algorithm.family() == Algorithm::MEOWPOW || ctx->algorithm.family() == Algorithm::EVRPROGPOW || ctx->algorithm.family() == Algorithm::MERAKI) && ((ctx->device_blocks < 0) || (ctx->device_threads < 0))) {
